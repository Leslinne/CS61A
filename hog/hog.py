...
def Hogtimus_Prime(original_prime_score):
    final_prime_score=original_prime_score+1
    while not is_prime(final_prime_score):
        final_prime_score+=1
    return final_prime_score
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 2
    if num_rolls==0:
        if is_prime(free_bacon(opponent_score)) and free_bacon(opponent_score) !=1:
            return Hogtimus_Prime(free_bacon(opponent_score))
        else:
            return free_bacon(opponent_score)
    else:
        current_score=roll_dice(num_rolls,dice)
        if is_prime(current_score) and current_score != 1:
            return Hogtimus_Prime(current_score)
        else:
...
def is_swap(score):
    ...
    third_digit=(score%100)%10
    if first_digit==second_digit==third_digit:
        return True
    elif first_digit==0 and second_digit==third_digit:
        return True
    elif first_digit==second_digit==0:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0<goal and score1<goal:
        ...
        if player==0:
            player0_turn=take_turn(min(max_dice(score0,score1),strategy0(score0,score1)),score1,dice)
            score0+=player0_turn
            if is_swap(score0):
                score0,score1=score1,score0
        if player==1:
            player1_turn=take_turn(min(max_dice(score0,score1),strategy1(score1,score0)),score0,dice)
            score1+=player1_turn
            if is_swap(score1):
                score0,score1=score1,score0
        player=other(player)
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    """
    # BEGIN PROBLEM 6
    for p in range (0,goal):
        for q in range (0,goal):
            num_rolls=strategy(p,q)
            check_strategy_roll(p,q,num_rolls)
    # END PROBLEM 6
    return strategy
...
def make_averaged(fn, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 7
    def average_whatever_function(*args):
        i=0
        results=[]
        while i<num_samples:
            results+=[fn(*args)]
        ...
        for y in results:
            sum_results+=y
        average_results=sum_results/(num_samples)
        return average_results
    return average_whatever_function
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def average_n_rolls(m):
        return make_averaged(roll_dice,num_samples)(n,dice)
    max_average=make_averaged(roll_dice,num_samples)(1,dice)
    k,n=1,1
    while n<10:
        n+=1
        if average_n_rolls(n)>max_average:
            max_average=average_n_rolls(n)
            k=n
    return k
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 9
    if_margin=take_turn(0,opponent_score,dice=select_dice(score,opponent_score))
    if if_margin >= margin:
        return 0
    else:
        return num_rolls
...
def swap_strategy(score, opponent_score, margin=5, num_rolls=6):
    ...
    # BEGIN PROBLEM 10
    if_margin=take_turn(0,opponent_score,dice=select_dice(score,opponent_score))
    update_score=score + if_margin
    if opponent_score > update_score and is_swap(update_score):
        return 0
    elif if_margin >= margin and not is_swap(update_score):
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 10
    if score<opponent_score and max_dice==10 and score>9 and how_much_to_unique(score)==1:
        return 10
    elif score<opponent_score:
        return swap_strategy(score, opponent_score, 11, 6)
    else:
        return swap_strategy(score, opponent_score, 5, 8)
