...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        distance = 0
        while place is not None and distance <= self.max_range :
            if random_or_none(place.bees) and self.min_range <= distance and place.name is not 'Hive' :
...
class ScubaThrower(ThrowerAnt):
    """Subclass of throwerant that is moire costly and watersafe"""
    name = 'Scuba'
    implemented = True
    watersafe = True
    food_cost = 6
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.ant:
            self.ant.action(colony)
        lst = [bee for bee in self.place.bees]
        for bee in lst:
            bee.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while location.exit:
            ...
            if location.ant and location.ant not in self.doubled:
                location.ant.damage=location.ant.damage*2
                self.doubled+=[location.ant]
            if location.ant and hasattr(location.ant,'ant') and location.ant.ant and location.ant.ant not in self.doubled:
                location.ant.ant.damage=location.ant.ant.damage*2
                self.doubled+=[location.ant.ant]
